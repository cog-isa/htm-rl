...
\author{
  \authorname{
    Petr Kuderov\sup{1}\orcidAuthor{0000-0001-7055-3426}, Aleksandr I. Panov\sup{1}\orcidAuthor{0000-0002-9747-3837}
  }
  \affiliation{
    \sup{1}Moscow Institute of Physics and Technology, Russia
  }
  \email{
    kuderov.pv@phystech.edu, panov.ai@mipt.ru
  }
}
...
\end{quote}
  ...
  \footnotetext{In the Sec.~\ref{sec_TM} we explained that TM can work with unions of SDR vectors. When SDR vectors in a union aren't just random vectors, when they represent states or actions, we call such unions \textit{superpositions} of states or actions respectively. Note that once a superposition vector is made you cannot simply split it back into the origin SDR vectors of the union. \footnotemark}

\footnotetext{However, our current encoding scheme maps the different states or actions with non-overlapping bits, which keeps them easily distinguishable from each other even in a superposition. Whilst it's true, the current planning algorithm only requires a separation of the state bits from the action bits in a state-action pair SDR. This is done intentionally. Non-overlapping scheme makes things simpler to read and debug. The downside is it cannot keep the similarity between states (or observations). That's why for the future work, when we plan to experiment with more complex environments, transition to overlapping mapping seems inevitable.}
...
\begin{figure}
  ...
  \caption{Detailed results for \texttt{multi\_way\_v0}. We compared two cases for the different planning horizons: tracking 16 rewarding goal states and only the last goal.} \label{fig_detailed_mw0}
\end{figure}
...
\begin{figure}
  ...
  \end{minipage}
  \caption{Episode durations for the experiment with rarely changed environments. a) Our agent with the planning horizon 8 performed the best. Each change of the environment configuration there's a spiked degradation of performance for DQN agent, while our agents quickly relearn new position. b) According to our results in this task $\epsilon$-greedy DQN agent adapts much faster. c-d) Comparison for the different planning horizon values and the number of simultaneously tracked goals. } \label{fig_rare_changes}
\end{figure}
...
\begin{figure}
  ...
  \caption{Episode durations for the experiment with frequently changed environments. a) Our method performed more stable with small planning horizons (1-4), while with planning horizon 8 it performed either very good or very bad. DQN agent results were worse than of random policy - the frequency of changes was too high for it to adapt in time. b) Unlike greedy DQN agent $\epsilon$-greedy agent performed at least comparable to our method. c-d) Comparison for the different planning horizon values and the number of simultaneously tracked goals. Larger goal tracking memory size degraded agent's performance. } \label{fig_frequent_changes}
\end{figure}
