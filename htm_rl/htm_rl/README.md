...
В рамках одной среды проводится серия испытаний по N эпизодов. В течение одного испытания из N эпизодов агент учится находить награду в некоторой ячейке. Между испытаниями местоположение награды изменяется. Исследуется способность агентов адаптироваться к новому местоположению награды.

### Постановка задачи transfer learning в нескольких средах

То же самое, но в рамках некоторого набора разных сред-лабиринтов:

- эксперимент проводится последовательно на `n_environments` средах
  - генерируются случайно на некотором квадрате размера `NxN`
- при фиксированной среде последовательно рассматриваются `n_terminal_states` положений награды
- при фиксированном положении награды рассматривается `n_initial_states` начальных положений агента
- в фиксированной конфигурации агент играет `n_episodes_all_fixed` эпизодов

Исследуется способность агентов
