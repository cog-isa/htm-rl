...
class Agent:
    ...
    def __init__(self, tm: TemporalMemory, encoder: SarSdrEncoder):
        self.tm = tm
    ...
    def predict_cycle(self, initial_input: SparseSdr, n_steps, print_enabled=False, reset_enabled=True):
        ...
        for i in range(n_steps):
            ...
            if print_enabled:
                sar_superposition = self.encoder.decode_sparse(proximal_input)
    ...
    def _str_from_cells(self, cells_sdr: SDR, name: str) -> str:
        ...
        for layer_ind in range(1, self.tm.cells_per_column):
            substrings.append(self.encoder.str_from_sparse(cells_sparse_sdr[layer_ind]))
        return '\n'.join(substrings)
