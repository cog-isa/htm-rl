...
class LegacyPlanner:
    ...
    def __init__(self, memory: LegacyMemory, planning_horizon: int):
        self.memory = memory
    ...
    def plan_actions(self, initial_sar: Sar, verbosity: int):
        planned_actions = []
        ...
        reward_reached = self._predict_to_reward(initial_sar, verbosity)
        if reward_reached:
            for activation_timeline in self._backtrack_from_reward(verbosity):
                planning_successful, planned_actions = self._check_activation_timeline_leads_to_reward(
                    initial_sar, activation_timeline, verbosity
                )
    ...
    def _backtrack_from_reward(self, verbosity: int):
        """
        ...
        self.memory.print_cells(verbosity, 3, depolarized_reward_cells, 'Reward == 1')
    ...
    def _backtrack(self, desired_depolarization: SparseSdr, t: int, verbosity: int) -> Tuple:
        ...
        for candidate_cluster, n_induced_depolarization in candidate_cell_clusters:
            ...
            backtracking_succeeded, activation_timeline = self._backtrack(candidate_cluster, t-1, verbosity)
            if backtracking_succeeded:
                activation_timeline.append(candidate_cluster)
