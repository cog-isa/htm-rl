...
- Добавить выученную модель среды
  ...
  - На этом этапе используются полностью определенные детерминированные простые MDP, так что ТМ попросту запоминает переходы.
- Усложнять среды (варианты в порядке постепенного усложнения)
  - более информативные и связанные состояния, используем кодирование с пересечениями
  - переходим к POMDP
  - добавление шума
  - стохастичность сред
